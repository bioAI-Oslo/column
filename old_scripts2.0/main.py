...
class NeuralNetwork_TF(tf.keras.Model):
    def __init__(self):
        ...
        self.dmodel = tf.keras.Sequential(
            [
                Flatten(input_shape=(3, 3, input_dim)),
                Dense(input_dim * 3 * 3, input_dim=input_dim * 3 * 3, activation="linear"),
                Dense(output_dim, activation="linear"),  # or linear
            ]
        )

        self(tf.zeros([1, 3, 3, input_dim]))  # dummy calls to build the model
...
def collect_input(input, img, perceptions, N_neo, M_neo):
    for x in range(N_neo):
        for y in range(M_neo):
            ...
            input[x * M_neo + y] = np.concatenate((perc, comms), axis=2)
...
def classify(network, img, perceptions, limits_list, N_active, M_active, N_neo, M_neo, visualize=False):
    ...
    for _ in range(ITERATIONS):
        ...
        input = np.empty((N_neo * M_neo, 3, 3, NUM_CHANNELS))
        collect_input(input, img, perceptions, N_neo, M_neo)
        # input[:,:,:,4:7] = 0.0

        guesses = network(np.array(input))
...
def evaluate_nca(
    ...
    for sample, (img_raw, expected) in enumerate(zip(training_data, target_data)):
        if sample != 0 and sample % 2 == 0:
            # Restart
            state = seed_state
            assert np.sum(np.abs(seed_state)) == 0, "Something is wrong"
            assert np.sum(np.abs(state)) == 0, "Something is wrong"
        ...
        for g in range(MAXGEN):
            ...
            if g % plotting_interval == 0 or visualize_this_gen:
                ...
                if THREADS > 1:
                    ...
                    loss_reduced_window, acc_reduced_window = np.mean(test_fitness[: THREADS // 2], axis=0)
                    loss_full_window, acc_full_window = np.mean(test_fitness[THREADS // 2 :], axis=0)
